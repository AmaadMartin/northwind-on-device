...
import type { SearchDoc, Candidate, ExpandedQuery } from './types.js';
...
function buildDoc(doc: SearchDoc): Doc {
  ...
  const urlWords = doc.url.replace(/https?:\/\//, '').replace(/[/\-_.?=&]+/g, ' ');
  const parts = [
  ...
  return { doc, tokens, tf, length: tokens.length };
}
...
export class Bm25Index {
  ...
  rebuild(docs: SearchDoc[]) {
    this.docs = docs.map(buildDoc);
    this.df.clear();
  ...
  score(query: ExpandedQuery): Candidate[] {
    ...
    for (const d of this.docs) {
      ...
      if (score > 0) out.push({ doc: d.doc, lexicalScore: score });
    }
  ...
  prefilter(query: ExpandedQuery, limit: number): Candidate[] {
    ...
    const seen = new Set(scored.map((c) => c.doc.id));
    const filler = this.docs
      .filter((d) => !seen.has(d.doc.id))
      .sort((a, b) => b.doc.indexedAt - a.doc.indexedAt)
      .slice(0, limit - scored.length)
      .map((d) => ({ doc: d.doc, lexicalScore: 0 }));
