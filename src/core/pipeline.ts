...
/** Sanitises a document id into a valid ADK agent name. */
let agentSeq = 0;
function agentNameFor(id: string): string {
  const safe = id.replace(/[^A-Za-z0-9_]/g, '_');
  return `rerank_${safe}_${agentSeq++}`;
}
...
export class TabSearchPipeline {
  ...
  private async *rerank(
    ...
    const children = candidates.map((c) => {
      const name = agentNameFor(c.doc.id);
    ...
    const fanout = new ParallelAgent({
      name: 'rerank_fanout',
      subAgents: children,
      description: 'Scores each candidate independently.',
    });
    ...
    for (const [name, cand] of byAgentName) {
      ...
      yield {
        doc: cand.doc,
        score: 0,
        why: 'the model returned no verdict for this item',
