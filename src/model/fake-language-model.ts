...
const ASSOCIATIONS: Record<string, string[]> = {
  ...
  // Northwind Hardware: shoppers describe symptoms, catalogues use trade names.
  slam: ['doorstop', 'stop', 'hinge', 'buffer', 'closer', 'cushion', 'door'],
  slamming: ['doorstop', 'stop', 'hinge', 'buffer', 'closer', 'cushion'],
  bang: ['doorstop', 'buffer', 'cushion', 'hinge'],
  drip: ['washer', 'cartridge', 'seal', 'valve', 'tap', 'plumbing', 'ceramic'],
  drips: ['washer', 'cartridge', 'seal', 'valve', 'tap', 'ceramic'],
  tap: ['cartridge', 'washer', 'valve', 'mixer', 'plumbing'],
  blowing: ['hook', 'cabin', 'latch', 'catch', 'gate', 'shed', 'galvanised'],
  shed: ['gate', 'hook', 'cabin', 'padbolt', 'galvanised'],
  hole: ['patch', 'filler', 'repair', 'plasterboard', 'mesh', 'skim'],
  plasterboard: ['patch', 'repair', 'mesh', 'filler', 'skim'],
  draught: ['sweep', 'seal', 'excluder', 'weatherproofing', 'brush', 'threshold'],
  draughty: ['sweep', 'seal', 'excluder', 'brush', 'threshold'],
  cold: ['draught', 'sweep', 'seal', 'excluder', 'insulation'],
  gap: ['sweep', 'seal', 'excluder', 'threshold', 'brush'],
  concrete: ['masonry', 'hammer', 'block', 'brick', 'drill', 'carbide'],
  leak: ['washer', 'seal', 'cartridge', 'sealant', 'valve'],
  rust: ['galvanised', 'stainless', 'weatherproof', 'zinc'],
  squeak: ['hinge', 'lubricant', 'oil'],
  wobbly: ['fixing', 'plug', 'anchor', 'bracket', 'screw'],
};
...
function pageText(text: string): string {
  const m = text.match(/^(?:DETAILS|PAGE TEXT):\s*([\s\S]*?)(?:\n\nScore this (?:page|item)\.|$)/mi);
  return m ? m[1].trim() : '';
}
...
function scorePage(promptText: string): { score: number; why: string } {
  const query = section(promptText, 'SHOPPER SAID') || section(promptText, 'QUERY');
  const title = section(promptText, 'ITEM') || section(promptText, 'PAGE TITLE');
...
function describe(
  ...
  if (!best || bestHits === 0) {
    // No clause actually matched. Echoing the title back reads as debug output
    // and claims a reason that was never found, so say nothing instead.
    return '';
  }
...
function chooseTool(promptText: string, constraint: any): string {
  ...
  const searched = lower.includes('[tool_result] searchproducts');

  // Always find real SKUs before doing anything with them.
  if (names.includes('searchProducts') && !searched) {
    const q = lastUser.replace(/^user:\s*/, '').trim();
    return JSON.stringify({ kind: 'tool', name: 'searchProducts', args: { query: q } });
  }

  // Stop once the work is visibly done, rather than re-issuing the same call.
  if (lower.includes('[tool_result] addtobasket')) {
    return JSON.stringify({ kind: 'final', text: 'Ready for you to confirm.' });
  }
  if (lower.includes('[tool_result] checkaisle')) {
    const aisle = promptText.match(/\\?"aisle\\?":\s*\\?"([^"\\]+)/);
    return JSON.stringify({
      kind: 'final',
      text: aisle ? `You will find it in aisle ${aisle[1]}.` : 'Here is what I found.',
    });
  }

  // SKUs come from the preceding searchProducts result, best match first.
  // Tolerate escaped quotes: tool results can arrive re-encoded.
  const skus = [...promptText.matchAll(/\\?"sku\\?":\s*\\?"(NW-\d+)/g)].map((m) => m[1]);

  if (
    names.includes('addToBasket')
    && /\b(add|buy|order|basket|cart|get me|i'll take)\b/.test(lower)
    && skus.length
  ) {
    const qty = lower.match(/\b(two|2)\b/) ? 2 : lower.match(/\b(three|3)\b/) ? 3 : 1;
    return JSON.stringify({
      kind: 'tool',
      name: 'addToBasket',
      args: { sku: skus[0], quantity: qty },
    });
  }
  if (
    names.includes('checkAisle')
    && /\b(aisle|where|find it|located)\b/.test(lower)
    && skus.length
  ) {
    return JSON.stringify({ kind: 'tool', name: 'checkAisle', args: { sku: skus[0] } });
  }
  return JSON.stringify({ kind: 'final', text: 'Here is what I found in the catalogue.' });
}
...
function respond(input: LanguageModelPrompt, constraint: any): string {
  ...
  if (props.keywords) {
    return JSON.stringify(expand(section(text, 'SHOPPER SAID') || section(text, 'QUERY') || text));
  }
  if (props.score) return JSON.stringify(scorePage(text));
  if (props.relevant) return JSON.stringify(judgeReview(text));
  if (props.findings) return JSON.stringify({ findings: findPii(text) });

  return 'This is simulated output from TabFind\'s fake model.';
}

/**
 * Judges one review against a question, for the review-scanning beat.
 *
 * Picks the sentence with the most shared terms and returns it verbatim, which
 * is what the real prompt asks the model to do. Sentiment comes from the star
 * rating, which is deterministic and always right.
 */
function judgeReview(promptText: string): {
  relevant: boolean;
  sentiment: string;
  quote: string;
} {
  const question = section(promptText, 'QUESTION');
  const m = promptText.match(/^REVIEW \((\d) stars\):\s*([\s\S]*?)(?:\n\nDoes this|$)/mi);
  if (!m) return { relevant: false, sentiment: 'none', quote: '' };
  const stars = Number(m[1]);
  const body = m[2].trim();

  // "is this any good on concrete" expands to include "good", which matches
  // half the reviews. Evaluative words are not evidence; only match on terms
  // that say something about the subject.
  const GENERIC = new Set(['good', 'bad', 'great', 'fine', 'nice', 'well', 'value',
    'quality', 'work', 'use', 'used', 'buy', 'bought', 'product', 'item']);
  const terms = new Set(
    expand(question).keywords.map(fold).filter((t) => !GENERIC.has(t)),
  );
  let best = '';
  let bestHits = 0;
  ...
  if (!best || bestHits === 0) return { relevant: false, sentiment: 'none', quote: '' };

  const sentiment = stars >= 4 ? 'positive' : stars <= 2 ? 'negative' : 'mixed';
  return { relevant: true, sentiment, quote: best };
}

/**
 * Finds names and street addresses, for the PII beat.
 *
 * Cards, emails and phone numbers are handled by the rule-based detectors in
 * pii.ts and are deliberately not duplicated here.
 */
function findPii(promptText: string): Array<{ kind: string; text: string }> {
  const m = promptText.match(/^TEXT:\n([\s\S]*?)\n\nList the personal/mi);
  const body = m ? m[1] : promptText;
  const out: Array<{ kind: string; text: string }> = [];

  // "42 Bridge Street", "1 High Road"
  for (const hit of body.matchAll(
    /\b\d{1,4}[a-z]?\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+(?:Street|Road|Lane|Avenue|Close|Drive|Way|Court|Place|Terrace)\b/g,
  )) {
    out.push({ kind: 'address', text: hit[0] });
  }
  // "I'm Dana Whitfield", "this is Sam Okafor"
  for (const hit of body.matchAll(
    /\b(?:I am|I'm|this is|my name is|name's)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)/g,
  )) {
    out.push({ kind: 'name', text: hit[1] });
  }
